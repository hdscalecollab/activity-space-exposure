...
full_dir = "C:/Users/jyang/Documents/TWSA_PA/Codes-R/Data/GPS_Acc_rfh/" # .csv files with GPS file (lon, lat)
...
ernelType = 'quartic'   # define the type of kernel to used for KDE/DR calculation  ('Gaussian', 'quartic'...)
...
for(i in 1:length(point_files)){
  ...
  #### ------------ Calculate Activity Space with DR ------------
...
#### ------------ Calculate Activity Space with DR Ends Here 
...
write.csv(df_err, paste0(paste("DR_errors_PtDates", cohort ,kernelType, target_radius_txt, cell_size, data_seg, sep="_"), '.csv'),  row.names = FALSE)
